PROGRAM FunctionRadiusMeanSquare
    IMPLICIT NONE
     ...
     !Agora precisamos construir a função, a qual será construída em um comando de laço.

     rB = 1.0

     PRINT *, 'rB =', rB

     DO WHILE (rB .LE. 2.0) !Aqui temos um laço que vai rodar uma certa equação, com rB como parâmetro, até que chegue no valor máximo estipulado.
        Rmq = (1.0/8.0)*((1.0/rKpp**2.0)-(1.0/rK)+ 2.0*(rC**2.0)+((2.0*rC+rB)*(1.0+rKpp*rB))/rKpp-((rB**3.0)*rKpp)/(3.0*(1.0+rB)))
